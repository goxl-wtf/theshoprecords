-- MARKETPLACE SCHEMA FIX SCRIPT (REVISED)
-- Run this script in Supabase SQL Editor to fix the marketplace schema issues

-- First, check which tables already have ID columns
DO $$
DECLARE
    has_seller_profiles_id BOOLEAN;
    has_listings_id BOOLEAN;
    has_orders_id BOOLEAN;
BEGIN
    -- Check if seller_profiles has id column
    SELECT EXISTS (
        SELECT 1 
        ...
        AND column_name = 'id'
    ) INTO has_seller_profiles_id;
    
    -- Check if listings has id column
    SELECT EXISTS (
        SELECT 1 
        FROM information_schema.columns 
        WHERE table_schema = 'public' 
        ...
        AND column_name = 'id'
    ) INTO has_listings_id;
    
    -- Check if orders has id column
    SELECT EXISTS (
        SELECT 1 
        FROM information_schema.columns 
        WHERE table_schema = 'public' 
    ...
    -- Add id column to seller_profiles if missing
    IF NOT has_seller_profiles_id THEN
        ALTER TABLE seller_profiles ADD COLUMN id UUID PRIMARY KEY DEFAULT gen_random_uuid();
        RAISE NOTICE 'Added id column to seller_profiles table';
    END IF;
    
    -- Raise notices for debugging
    RAISE NOTICE 'Seller profiles has id: %', has_seller_profiles_id;
    RAISE NOTICE 'Listings has id: %', has_listings_id;
    RAISE NOTICE 'Orders has id: %', has_orders_id;
END$$;
    ...
    IF EXISTS (
        ...
        IF NOT EXISTS (
            ...
            ALTER TABLE seller_orders 
              ...
              ON DELETE CASCADE;
            RAISE NOTICE 'Added foreign key constraint between seller_orders and seller_profiles';
        END IF;
    ELSE
        RAISE NOTICE 'Could not add foreign key for seller_orders: seller_profiles.id does not exist';
    END IF;
END$$;
